...
// const Withdrawl = ({ isOpen, onClose, account }) => {
...
//                       aria-labelledby="__BVID__3017___BV_tab_button__"
//                     >
//                       <form action="#">
...
//                         </div>
//                         <div className="form-group row">
//                           <label className="col-form-label col-4">{account?.username}</label>
//                           <div className="col-8">
//                             <div className="row">
//                               <div className="col-6">
//                                 <input
...
//                                   className="form-control txt-right"
//                                   type="text"
//                                   defaultValue={account?.pts ? account.pts.replace(/,/g, '') : ''}
...
const withdrawChips = async ({
  ...
  const response = await fetch(
    `${import.meta.env.VITE_SERVER_URL}/api/v1/withdraw`,
    {
      method: "POST",
      headers: {
...
const Withdrawl = ({ isOpen, onClose, account }) => {
  ...
  const withdrawMutation = useMutation(withdrawChips, {
    onMutate: () => {
      setIsLoading(true);
    },
    onSuccess: (data) => {
      setIsLoading(false);
      if (data.success) {
        SuccessPopup(data.msg || "Withdrawal successful").then(() => {
          setFormData({ amount: "", remark: "", transactionCode: "" });
          onClose();
        });
      } else {
        ErrorPopup(data.error || data.msg || "Withdrawal failed", 2000);
      }
    },
    onError: (error) => {
      setIsLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    const remark = formData.remark || `Withdraw from ${username}`;
    ...
    if (Number(amount) > currentBalance) {
      ErrorPopup("Insufficient balance", 2000);
      return;
    }
                    ...
                    <ul role="tablist" className="nav nav-tabs">
                      <li role="presentation" className="nav-item">
                        <a
                          ...
                          href="#"
                          target="_self"
                          className="nav-link active tab-bg-danger"
                        >
                          Withdraw
                        </a>
                      </li>
