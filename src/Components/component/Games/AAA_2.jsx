...
const AmarAkbarAnthony_2 = ({
      ...
      {!bet && (
        <div className="flex relative w-full h-full">
          <div className="center-container">
            ...
            <div className="aaa casino-detail">
              <div className="casino-table">
                {/* Top Section - Amar, Akbar, Anthony */}
                <div className="casino-table-box">
                  {casinoData?.data?.data?.data?.t2?.slice(0, 3).map((item, index) => (
                ...
                <div className="casino-table-box mt-3">
                  <div className="casino-table-left-box">
                    {casinoData?.data?.data?.data?.t2
                      ...
                      .map((item) => (
                        <div className="aaa-odd-box" key={item.sid}>
                  ...
                  <div className="casino-table-right-box">
                    {casinoData?.data?.data?.data?.t2
                      ?.filter((v) => v.nat.includes("Under") || v.nat.includes("Over"))
                      .map((item) => (
                        <div className="aaa-odd-box" key={item.sid}>
                          <div className="casino-odds text-center">
                            {remainingTime <= 3 || item.gstatus === "SUSPENDED" ? "0" : item.b1}
                          ...
                          {remainingTime <= 3 || item.gstatus === "SUSPENDED" ? (
                            ...
                            <div
                              className="casino-odds-box back casino-odds-box-theme cursor-pointer hover:opacity-90"
                              onClick={() => handleItemSelect('range', item.nat, {
                                betName: item?.nat,
                                matchOdd: item?.b1,
                                sid: item?.sid
                              })}
                ...
                <div className="casino-table-full-box aaacards mt-3">
                  <h4 className="w-100 text-center mb-2">
                    <b>
                      {casinoData?.data?.data?.data?.t2
                        ?.filter((v) => v.nat.includes("Card") && !v.nat.includes("HIGH") && !v.nat.includes("LOW"))
                        ?.map((item, index) => index === 0 && (remainingTime <= 3 || item.gstatus === "SUSPENDED" ? "0" : item.b1))}
                    </b>
                  </h4>
                  <div className="flex flex-wrap gap-2 justify-center">
                    {casinoData?.data?.data?.data?.t2
                      ?.filter((v) => v.nat.includes("Card") && !v.nat.includes("HIGH") && !v.nat.includes("LOW"))
                      .map((item, index) => {
                        const card = item.nat.replace("Card", "").trim();
                        return (
                          <div className="card-odd-box" key={index}>
                            {remainingTime <= 3 || item.gstatus === "SUSPENDED" ? (
                              <div className="suspended-box">
                                <img
                                  src={cardVariant.find((v) => v.code === card)?.image}
                                  alt={card}
                                />
                              </div>
                            ) : (
                              <div
                                ...
                                onClick={() => handleItemSelect('card', item.nat, {
                                  betName: item?.nat,
                                  matchOdd: item?.b1,
                                  sid: item?.sid
                                })}
  ...
  {/* ONLY ONE CARD — CENTER ME SHOW */}
                                      ...
                                      <div className="col-md-12 text-center">
                                        {(() => {
                                          const cardCode = individualResultData?.cards;  // "ASS"
...
export default AmarAkbarAnthony_2;