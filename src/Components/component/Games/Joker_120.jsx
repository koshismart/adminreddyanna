...
const Joker_120 = ({
  ...
  const jokerCards = Array.from({ length: 13 }, (_, i) => i + 1);
  const {
    placeBet,
    setPlaceBet,
    betData,
      ...
      {!bet && (
        <div className="flex relative w-full h-full gap-1">
          <div className="center-container">
            ...
            <div className="teenpatti-joker1 casino-detail">
              <div className="casino-table">
                <div className="casino-table-box">
                  <div className="joker1-box">
                    <div className="joker1-other-cards">

                      {/* YE CONDITION LAGA DIYA – SUSPENDED YA TIME KAM TO CARD HIDE */}
                      {remainingTime > 3 && 
                      oddsData.some(item => item.gstatus !== "SUSPENDED" && item.gstatus !== "2") && (
                        <>
                          <h4 className="text-white text-center mb-3 font-semibold">Select your Joker</h4>
                          <div className="flex flex-wrap justify-center gap-1 px-2">
                            {jokerCards.map((num) => (
                              <img
                                key={num}
                                src={`https://g1ver.sprintstaticdata.com/v78/static/front/img/joker1/${num}.png`}
                                alt={`Joker ${num}`}
                                className="cursor-pointer hover:brightness-110 transition-all"
                                onClick={() => {
                                  if (remainingTime <= 3) return;
                                  setSelectedJoker(num);
                                }}
                              />
                            ))}
                          </div>
                      ...
                      {/* Jab SUSPENDED ya time khatam → kuch nahi dikhega (ya message dikha sakta hai optional) */}
                      {remainingTime <= 3 || oddsData.some(item => item.gstatus === "SUSPENDED" || item.gstatus === "2") ? (
                        <div className="text-center py-8">
                          <p className="text-red-500 font-bold text-lg animate-pulse">
                            {remainingTime <= 3 ? "Betting Closed!" : "Market Suspended"}
                          </p>
                        </div>
                      ) : null}
                    ...
                    <div className="casino-table-body">
                      {oddsData
                        ...
                        .map((item, idx) => (
                          <div
                            key={idx}
                            className={`casino-table-row ${remainingTime <= 3 || item.gstatus == "SUSPENDED"
                                ? "suspended-row"
                                : ""
                              }`}
                          >
                              ...
                              {remainingTime <= 3 || item.gstatus == "SUSPENDED" ? (
                                ...
                                <span
                                  ...
                                  const newBetData = {
                                    ...
                                    oddType: "casino_odds",
                                    oddCategory: "Back",
                                    selectedJoker: selectedJoker,  // Yeh ja raha hai backend ko
                                  };
